...
import { ClipboardList, ChevronDown, ChevronUp, IndianRupee, Calendar, ShieldCheck, Clock3 } from "lucide-react";
...
function HistoryCard({ item, index }) {
  ...
  const amtColor = item.status === "Approved" ? C.green : item.status === "Rejected" ? C.red : C.orange;

  return (
    <motion.div {...fu(index * 0.08)} style={{ ...card(), overflow:"hidden" }}>
      <div style={{ padding:"18px 20px", cursor:"pointer", userSelect:"none" }} onClick={() => setOpen(!open)}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            <div style={{ width:44, height:44, borderRadius:14, flexShrink:0, background:`${amtColor}16`, display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <span style={{ padding:"4px 12px", borderRadius:20, fontSize:12, fontWeight:700, color:cfg.color, background:cfg.bg, border:`1px solid ${cfg.border}` }}>
              {item.status}
            </span>
      ...
      {open && (
        <motion.div initial={{ height:0, opacity:0 }} animate={{ height:"auto", opacity:1 }} transition={{ duration:0.3 }} style={{ borderTop:`1px solid ${C.border}`, overflow:"hidden" }}>
          <div style={{ padding:"18px 20px", display:"flex", gap:20, flexWrap:"wrap" }}>
            <div style={{ flex:2, minWidth:240 }}>
              <div style={{ fontSize:12, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8 }}>AI Decision Message</div>
              <p style={{ fontSize:14, color:C.text1, lineHeight:1.7, margin:0 }}>{item.sellerMsg}</p>
              <div style={{ marginTop:12, fontSize:12, color:C.muted }}>Documents uploaded: {item.documentCount ?? 0}</div>
            </div>
            <div style={{ flex:1, minWidth:200 }}>
              <div style={{ fontSize:12, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:8 }}>Improvement Plan</div>
              {item.plan.map((p, i) => (
                <div key={i} style={{ display:"flex", gap:8, alignItems:"flex-start", marginBottom:8 }}>
                  <div style={{ width:20, height:20, borderRadius:6, flexShrink:0, background:"rgba(111,45,189,0.1)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:800, color:C.purple }}>{i + 1}</div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <motion.div {...fu(0)}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
          <div style={{ width:44, height:44, borderRadius:14, background:"rgba(111,45,189,0.1)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <ClipboardList style={{ width:22, height:22, color:C.purple }} />
          </div>
          <div>
            <div style={{ fontSize:20, fontWeight:800, color:C.text1 }}>Loan History</div>
            <div style={{ fontSize:13, color:C.muted }}>{seller.name} · {history.length} application{history.length !== 1 ? "s" : ""} on record</div>
          </div>
        </div>
      </motion.div>

      <motion.div {...fu(0.06)} style={{ ...card(), padding:18, background:"linear-gradient(135deg, #F8F4FF 0%, #ffffff 100%)" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <div>
            <div style={{ fontSize:12, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.06em" }}>Portfolio Snapshot</div>
            <div style={{ fontSize:16, fontWeight:800, color:C.text1, marginTop:4 }}>Healthy application flow with clear approval momentum</div>
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {[
              { label:"Approval rate", value:`${history.length ? Math.round((approved / history.length) * 100) : 0}%`, icon:ShieldCheck, color:C.green },
              { label:"Avg. review", value:"2-3 days", icon:Clock3, color:C.orange },
            ].map((item, idx) => {
              const Icon = item.icon;
              return (
                <div key={idx} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 12px", borderRadius:999, background:"rgba(111,45,189,0.06)", border:`1px solid ${C.border}` }}>
                  <div style={{ width:28, height:28, borderRadius:999, background:`${item.color}16`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <Icon style={{ width:14, height:14, color:item.color }} />
                  </div>
                  <div style={{ fontSize:12, color:C.muted }}>{item.label}<br /><span style={{ color:C.text1, fontWeight:700 }}>{item.value}</span></div>
                </div>
              );
            })}
          </div>
      ...
      <motion.div {...fu(0.06)} style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:12 }}>
        {[{ label:"Total Applications", value:history.length, color:C.purple }, { label:"Approved", value:approved, color:C.green }, { label:"In Review", value:review, color:C.orange }, { label:"Rejected", value:rejected, color:C.red }].map((s) => (
          <div key={s.label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"16px 20px", textAlign:"center", boxShadow:"0 2px 10px rgba(111,45,189,0.06)" }}>
      ...
      {history.length === 0 ? (
        <motion.div {...fu(0.1)} style={{ ...card({ padding:40 }), textAlign:"center" }}>
          ...
          <div style={{ fontSize:14, color:C.muted }}>{seller.name} hasn't submitted any loan applications yet.</div>
        </motion.div>
      ) : (
